...
class PaymentRiskEngine:
    ...
    def load(self) -> None:
        from bgi_trident.graph.payment_builder import (
            PaymentGraphBuilder, PaymentInteractionData,
        )
    ...
    def assess_payment_risk(
        ...
        ensemble = (
            0.25 * p1_payer["score"] +
            0.20 * p1_merchant["score"] +
            0.55 * p2["score"]
        )
        ensemble = round(min(ensemble, 1.0), 4)

        decision = (
            "BLOCK"  if ensemble >= 0.55 else
            "REVIEW" if ensemble >= 0.30 else
            "ALLOW"
        )
        ...
        return {
            ...
            "explanation":    _build_explanation(
                decision, ensemble, p1_payer, p1_merchant, p2,
    ...
    def detect_merchant_ring(
        ...
        bank_peers = [
            m for m in _shared_bank_merchants(b, merchant_id)
            if m != merchant_id
        ]
        ...
        if high:
            summary_lines.append(
                f"RING DETECTED: Merchant {merchant_id} has {len(high)} HIGH-strength ring partner(s)."
            )
        if bank_peers:
            summary_lines.append(
                f"SHARED BANK: {len(bank_peers)} other merchant(s) share the same settlement account."
            )
        if refund_rate > 0.35:
    ...
    def generate_dispute_evidence(
        ...
        b = self._builder
        payer_txns     = b.payer_txns.get(payer_id, [])
        prior_to_merch = [t for t in payer_txns
                          if t.get("merchant_id") == merchant_id
                          and t.get("payment_id") != payment_id]
        target_txn     = next(
            (t for t in payer_txns if t.get("payment_id") == payment_id), None
        )
        ...
        evidence_strength = (
            "STRONG"   if len(prior_to_merch) >= 2 and device_chain else
            "MODERATE" if (prior_to_merch or device_chain) else
            "WEAK"
        )
        ...
        return {
            ...
            "narrative": _dispute_narrative(
                payment_id, merchant_id, payer_id, amount, reason,
                prior_to_merch, device_chain,
            ),
        }
...
def _velocity_score(payer_id, amount, txns, now) -> dict:
    ...
    def vel(hours):
        return sum(
            1 for t in txns
            if (now - datetime.fromisoformat(str(t["created_at"]))).total_seconds() <= hours * 3600
        )

    v1h, v6h, v24h = vel(1), vel(6), vel(24)
    details.update(velocity_1h=v1h, velocity_6h=v6h, velocity_24h=v24h)

    if v1h  >= 5:  features.append(f"HIGH_VELOCITY_1H:{v1h}")
    if v6h  >= 15: features.append(f"HIGH_VELOCITY_6H:{v6h}")
    if v24h >= 30: features.append(f"HIGH_VELOCITY_24H:{v24h}")
    ...
    if micro  >= 10: features.append(f"MICRO_TXN_BURST:{micro}")
    ...
    if len(amounts) >= 3:
        ...
        if z > 3.5: features.append(f"AMOUNT_OUTLIER:z={z:.1f}")

    score = min(sum({
        "HIGH_VELOCITY_1H": 0.30, "HIGH_VELOCITY_6H": 0.20,
        "HIGH_VELOCITY_24H": 0.15, "MICRO_TXN_BURST": 0.40,
        "HIGH_FAILED_RATE": 0.25, "AMOUNT_OUTLIER": 0.20,
    }.get(f.split(":")[0], 0) for f in features), 1.0)
...
def _merchant_score(merchant_id, txns) -> dict:
    ...
    score = min(sum({"HIGH_REFUND_RATE": 0.35, "HIGH_REFUND_VALUE_RATIO": 0.30}
                    .get(f.split(":")[0], 0) for f in features), 1.0)
...
def _shared_bank_merchants(b, merchant_id) -> list[str]:
    ...
    try:
        bank = b.data.merchants[
            b.data.merchants["merchant_id"] == merchant_id
        ]["bank_account"].iloc[0]
        peers = b.data.merchants[
            b.data.merchants["bank_account"] == bank
        ]["merchant_id"].tolist()
...
def _detect_rings(b, merchant_id, min_shared=3) -> list[dict]:
    ...
    target_payers = set(
        t["payer_id"] for t in b.merchant_txns.get(merchant_id, [])
    )
    ...
    for co_mid, cnt in co_map.items():
        ...
        rings.append({
            ...
            "ring_strength": (
                "HIGH"   if shares_bank and cnt >= 5 else
                "MEDIUM" if cnt >= 3 else
                "LOW"
            ),
        })
...
def _graph_score(b, merchant_id, payer_id) -> dict:
    ...
    if bank_peers:
        signals.append(
            f"SHARED_BANK_ACCOUNT: merchant {merchant_id} shares bank with "
            f"{len(bank_peers)} merchant(s): {bank_peers[:3]}"
        )
    # Device sharing
    payer_dev = None
    try:
        payer_dev = b.data.payers[
            b.data.payers["payer_id"] == payer_id
        ]["device_id"].iloc[0]
    except (IndexError, KeyError):
        pass
    if payer_dev:
        ...
        if len(co_payers) >= 2:
            signals.append(
                f"SHARED_DEVICE: payer {payer_id} shares device with {len(co_payers)} other payer(s)"
            )
    ...
    if high_rings:
        signals.append(
            f"MERCHANT_RING_HIGH: {len(high_rings)} HIGH-strength ring partner(s) for {merchant_id}"
        )
    ...
    s = 0.0
    for sig in signals:
        if   "SHARED_BANK"          in sig: s += 0.30
        elif "MERCHANT_RING_HIGH"   in sig: s += 0.50
        elif "MERCHANT_RING_MEDIUM" in sig: s += 0.25
        elif "SHARED_DEVICE"        in sig: s += 0.25
        elif "REFUND_CYCLE"         in sig: s += 0.40
    return {"score": round(min(s, 1.0), 3), "signals": signals, "rings": rings}


def _build_explanation(decision, score, p1_payer, p1_merchant, p2,
                       payment_id, merchant_id, payer_id, amount) -> str:
    lines = [
    ...
    if high_rings:
        ...
        for r in high_rings[:3]:
            lines.append(
                f"  - {r['ring_merchant']}  |  shared_payers={r['shared_payer_count']}  |  shared_bank={r['shares_bank_account']}"
            )
        lines.append("")
...
def _dispute_narrative(payment_id, merchant_id, payer_id, amount,
                       reason, prior_txns, device_chain) -> str:
    lines = [
...
def _generate_seed_data(data_dir: Path) -> None:
    ...
    for i in range(2000):
        ...
        rows.append({
            ...
            "status":      "refunded" if ring and random.random() < 0.4 else random.choices(["captured", "failed", "refunded"], weights=[80, 10, 10])[0],
