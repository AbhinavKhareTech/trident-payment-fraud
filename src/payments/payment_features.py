...
class PaymentFraudFeatureExtractor:
    ...
    def extract_merchant_features(self) -> pd.DataFrame:
        ...
        bank_counts = (
            self.merchants.groupby("bank_account")["merchant_id"]
            .transform("count")
            .rename("merchants_sharing_bank")
        )
    ...
    def extract_pair_features(self) -> pd.DataFrame:
        ...
        pair_agg = pair_agg.merge(
            payer_feats.add_prefix("payer_"), left_on="payer_id", right_index=True, how="left"
        ).merge(
        ...
        labels = (
            self.txns.groupby(["payer_id", "merchant_id"])["fraud_label"]
            .max()
            .reset_index()
