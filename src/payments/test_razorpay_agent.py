...
class MockBGIAllow(MCPServer):
    ...
    def available_tools(self):  return ["assess_payment_risk", "detect_merchant_ring", "generate_dispute_evidence"]
    async def call_tool(self, tool_name, params):
        if tool_name == "assess_payment_risk":
            return MCPToolResult(tool_name=tool_name, success=True, data={
                "decision": "ALLOW", "ensemble_score": 0.15,
                "explanation": "No signals detected.", "graph_signals": [],
                "rings_detected": [],
            })
        if tool_name == "detect_merchant_ring":
            return MCPToolResult(tool_name=tool_name, success=True, data={
                "ring_partners": [], "summary": "No ring detected.",
            })
...
class MockBGIBlock(MCPServer):
    ...
    async def connect(self):    pass
    async def close(self):      pass
    def available_tools(self):  return ["assess_payment_risk"]
    async def call_tool(self, tool_name, params):
        return MCPToolResult(tool_name=tool_name, success=True, data={
...
class MockRazorpay(MCPServer):
    ...
    async def connect(self):    pass
    async def close(self):      pass
    def available_tools(self):  return ["create_payment_link", "capture_payment", "fetch_dispute"]
    async def call_tool(self, tool_name, params):
        self.calls.append((tool_name, params))
        return MCPToolResult(tool_name=tool_name, success=True, data={
            "id": "plink_TEST", "status": "created", "short_url": "https://rzp.io/i/TEST"
        })
...
async def test_create_payment_link_allow(razorpay_mock):
    ...
    result = await agent.create_payment_link(
        merchant_id="mrc_00050", payer_id="pay_00300",
        amount=1499.0, description="Test payment",
    )
...
async def test_create_payment_link_blocked(razorpay_mock):
    ...
    result = await agent.create_payment_link(
        merchant_id="mrc_00005", payer_id="pay_00025",
        amount=45000.0, description="Ring B merchant",
    )
...
async def test_execute_order_allow(razorpay_mock):
    ...
    result = await agent.execute_order(
        payment_id="pay_001", merchant_id="mrc_00050",
        payer_id="pay_00300", amount=1499.0,
    )
...
async def test_execute_order_blocked(razorpay_mock):
    agent  = RazorpayFraudAgent(razorpay_mock, MockBGIBlock())
    result = await agent.execute_order(
        payment_id="pay_001", merchant_id="mrc_00005",
        payer_id="pay_00025", amount=45000.0,
    )
...
async def test_generate_dispute_evidence(razorpay_mock):
    ...
    result = await agent.generate_dispute_evidence(
        payment_id="pay_001", dispute_id="disp_001",
        merchant_id="mrc_00001", payer_id="pay_00005",
        amount=12500.0, reason="customer_dispute",
    )
    assert result.success
